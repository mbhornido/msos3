...
@include('includes.top_header_user')
            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="hero__categories">
                        ...
                        <ul>
                                <!-- Loop through departments (if needed) -->
                        @foreach($allDepartments as $departmentItem)
                            <li>
                                <a href="{{ url('seller_departments', $departmentItem->id) }}">
    ...
    <section class="product spad center_product">
        ...
        <section class="seller-details">
            <div class="seller-info">
                ...
                <div>
                    <h3>All Sellers of {{ $department->department_name }} </h3>
    ...
    <!-- <h1>{{ $department->department_name }} Shops</h1>
    <p>Showing admin accounts from {{ $department->department_name }}</p> -->

    <section class="product-grid">
        @foreach($admins as $admin)
            <div class="product-card">
                <img src="{{ asset('logo/' . $admin->logo) }}" alt="{{ $admin->shop_name }} Logo">
                <h5>{{ $admin->shop_name }}</h5>
                <p>{{ $admin->college }}</p>
                <a href="{{ url('view_seller_profile/' . $admin->id) }}" class="secondary-button">
